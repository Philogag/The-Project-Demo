  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getValues = computed(() => {
        ...
        if (isArray(value)) {
          const separators = editComponentProps?.separators ?? ' | ';

          let selectionOptions: any[] = [];
          if (editComponentProps?.labelInValue) {
            selectionOptions = value;
          } else {
            value.forEach((valueItem) => {
              selectionOptions.push(options.find((item) => `${item.value}` === `${valueItem}`));
            });
          }
          return selectionOptions.map((item) => item?.label).join(separators);
        } else {
          if (editComponentProps?.labelInValue){
            return value.label;
          } else {
      ...
      async function handleSubmitRule() {
        ...
        if (editRule) {
          if (isBoolean(editRule) && !currentValue) {
